...
const ProviderDashboard = () => {
  ...
  useEffect(() => {
    if (!user || (user.role !== 'walker' && user.role !== 'daycare')) {
      navigate('/dashboard');
      return;
    }
    fetchData();
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-heading font-bold text-stone-900 mb-2">
              Panel de {user?.role === 'walker' ? 'Paseador' : 'Guardería'}
            </h1>
                ...
                {inbox.map((item) => (
                  <Card 
                    ...
                    className={`rounded-2xl border-2 transition-all ${
                      item.is_expired ? 'border-stone-200 opacity-60' : 'border-emerald-200 hover:border-emerald-300'
                    }`}
